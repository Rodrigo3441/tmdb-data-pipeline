...
from src.genres.silver import transform
...
def run():
    ...
    bronze_data = silver_extract.execute(engine, 'bronze', 'genres')

    logger.info('Transforming the bronze data')
    silver_data = transform.run(bronze_data['genres'])

    logger.info('Loading the silver data into the database')
    data_was_loaded = load.execute(engine, silver_data, 'silver')

    if data_was_loaded:
        logger.info('Genres data loaded into the silver layer successfully.')
        
    else:
        logger.error('Failed to load the Genres data into the database')
